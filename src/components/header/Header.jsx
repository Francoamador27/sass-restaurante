...
import { Menu, X, ChevronDown, Sparkles, Zap, LogIn, User, LogOut, Settings } from "lucide-react";
          ...
          <div className="flex justify-between items-center h-20">
            ...
            <Link to="/" className="flex items-center space-x-3 group">
              ...
              <div className="flex flex-col">
                <span className={`font-black text-xl transition-colors ${scrolled ? 'text-slate-900' : 'text-white'
                  }`}>
                  DentalCor
                </span>
                <span className={`text-xs font-semibold transition-colors ${scrolled ? 'text-blue-600' : 'text-blue-100'
                  }`}>
            ...
            <ul className="hidden lg:flex items-center space-x-1">
              {navItems.map((item, index) => (
                <li key={index}>
                  <Link
                    to={item.href}
                    className={`px-5 py-2.5 rounded-xl font-semibold transition-all duration-200 ${scrolled
            ...
            <div className="hidden lg:flex items-center space-x-4">
              {/* Badge promocional */}
              <div className={`hidden xl:flex items-center gap-2 px-4 py-2 rounded-full transition-all ${scrolled
                ...
                <Sparkles className={`w-4 h-4 ${scrolled ? 'text-yellow-600' : 'text-yellow-300'}`} />
              ...
              {!user ? (
                <div className="flex items-center gap-3">
                  ...
                  <Link
                    to="/registro"
                    className="group relative bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white px-6 py-2.5 rounded-xl font-bold transition-all duration-200 flex items-center gap-2 shadow-lg hover:shadow-emerald-500/50 overflow-hidden"
                  >
                ...
                <div className="relative">
                  <button
                    ...
                    className={`flex items-center gap-3 px-5 py-2.5 rounded-xl font-semibold transition-all duration-200 ${scrolled
                        ...
                        <div className="bg-gradient-to-r from-blue-600 to-cyan-600 px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-blue-100 text-sm">{user?.email || 'usuario@email.com'}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="py-2">
                          ...
                          >
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">Ver perfil y datos</p>
                            </div>
                          </Link>
            ...
            <button
              ...
              className={`lg:hidden p-2.5 rounded-xl transition-all ${scrolled
              ...
              <ul className="space-y-2 mb-6">
                {navItems.map((item, index) => (
                  <li key={index}>
                    <Link
                      to={item.href}
                      className={`block px-4 py-3 rounded-xl font-semibold transition-all ${scrolled
              ...
              <div className="space-y-3 px-4">
                {!user ? (
                  <>
                    <Link
                      to="/auth/login"
                      className={`block text-center px-5 py-3 rounded-xl font-semibold transition-all ${scrolled
                    ...
                    <Link
                      to="/registro"
                      className="block text-center bg-gradient-to-r from-emerald-500 to-emerald-600 text-white px-5 py-3 rounded-xl font-bold shadow-lg hover:shadow-emerald-500/50 transition-all"
                      onClick={() => setMobileMenuOpen(false)}
                    >
                  ...
                  <div className={`rounded-xl p-4 ${scrolled ? 'bg-slate-100' : 'bg-white/10 backdrop-blur'
                    }`}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <p className={`font-bold ${scrolled ? 'text-slate-900' : 'text-white'}`}>
                          {user?.name || 'Usuario'}
                        </p>
                        <p className={`text-sm ${scrolled ? 'text-slate-600' : 'text-white/70'}`}>
                    ...
                    <div className="space-y-2">
                      <Link
                        to="/mi-cuenta"
                        className={`block px-4 py-2.5 rounded-lg font-semibold transition-all ${scrolled
